...
class FaceIdentifier {
  static Future<DetectedFace?> scanImage({required CameraImage cameraImage, required CameraController? controller, required FaceDetectorMode performanceMode, double? faceSizeThreshold, double? centerMargin}) async {
    final orientations = {
    ...
    DetectedFace? result;
    final face = await _detectFace(performanceMode: performanceMode, visionImage: _inputImageFromCameraImage(cameraImage, controller, orientations), imageSize: Size(cameraImage.width.toDouble(), cameraImage.height.toDouble()), threshold: faceSizeThreshold, centerMargin: centerMargin);
    if (face != null) {
      result = face;
    }
  ...
  static Future<DetectedFace?> _detectFace({required InputImage? visionImage, required FaceDetectorMode performanceMode, required Size imageSize, double? threshold, double? centerMargin}) async {
    if (visionImage == null) return null;
    final options = FaceDetectorOptions(enableLandmarks: true, enableTracking: true, enableClassification: true, performanceMode: performanceMode);
  ...
  static DetectedFace? _extractFace(List<Face> faces, Size imageSize, {double threshold = 0.1, double centerMargin = 0.4}) {
    ...
    for (Face face in faces) {
      ...
      bool isCentered = (faceCenterX >= imageCenterX - xMargin && faceCenterX <= imageCenterX + xMargin) && (faceCenterY >= imageCenterY - yMargin && faceCenterY <= imageCenterY + yMargin);
    ...
    return DetectedFace(wellPositioned: wellPositioned, face: bestFace);
  }
  ...
  static bool hasAllLandmarks(Face face) {
    ...
    return face.landmarks[FaceLandmarkType.leftEye] != null && face.landmarks[FaceLandmarkType.rightEye] != null && face.landmarks[FaceLandmarkType.noseBase] != null && face.landmarks[FaceLandmarkType.leftMouth] != null && face.landmarks[FaceLandmarkType.rightMouth] != null;
  }
}
